...
SELECT s.s_reg, s.s_fname, s.s_sname, ss.s_code, ss.year_reg FROM student s, student_subject ss 
WHERE s.s_reg = ss.s_reg AND ss.year_reg = '2010';

SELECT s_fname AS First_Name, s_sname AS Surname FROM student;

SELECT s.s_reg, s.tel_no, ss.s_code FROM student s INNER JOIN student_subject ss 
ON s.s_reg = ss.s_reg WHERE s_code = 'EED001';

SELECT MIN(year_reg) AS Oldest_Reg_Year FROM student_subject; 

SELECT DISTINCT c.s_code, s.s_title FROM subject s RIGHT JOIN classschedule c 
ON s.s_code = c.s_code WHERE c_day = 'Saturday' OR c_day = 'Sunday';

SELECT c.s_code, s.s_title, c.c_day, c.classhall FROM subject s INNER JOIN classschedule c 
ON s.s_code = c.s_code WHERE c_day = 'Sunday' AND classhall='01';

SELECT s_code,classdate, c_day FROM classschedule WHERE c_day <> 'Sunday';

SELECT s_reg, s_fname, length(s_fname) FROM student;

SELECT s_code, COUNT(s_reg) FROM student_subject GROUP BY s_code;

SELECT classhall,COUNT(s_reg) AS Student_count FROM classattendance 
WHERE s_code = 'EED001' GROUP BY classhall;

SELECT s.s_reg, s_fname, s_sname FROM student s, classattendance c
WHERE s.s_reg = c.s_reg AND attendance='Yes';

SELECT s_reg, COUNT(s_reg) AS Count_Reg_Subjects FROM student_subject GROUP BY s_reg HAVING COUNT(s_reg)>1;
